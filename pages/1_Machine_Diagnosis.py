...
with b:
    ...
    st.markdown(
        f"<div class='sp-card'><div class='sp-title'>Inference</div><h4 style='color:#8B5CF6;'>{result['top_failure']}</h4><p style='font-size:12px; color:#6B7280;'>Sev: {result['severity']}</p></div>",
        unsafe_allow_html=True
    )

    st.markdown(
        f"<div class='sp-card'><div class='sp-title'>ML Prediction</div><h4 style='color:#EC4899;'>{result['ml_class']}</h4><p style='font-size:12px; color:#6B7280;'>Conf: {result['ml_confidence']*100:.1f}%</p></div>",
        unsafe_allow_html=True
    )
...
with c:
    ...
    ) if result["recommendations"] else "Monitor normally."

    st.markdown("##### Action Items")

    st.markdown(
        f"<div class='sp-card' style='font-size:12px;'>{recs}</div>",
        unsafe_allow_html=True
    )
...
breakdown = pd.DataFrame([
    ...
    for k, v in result["per_failure_scores"].items()
